...
public sealed class CpbAnalyzerTests
{
    ...
    public void AWeightingReducesLowFrequencyBandAndRecordsMetadata()
    {
        ...
        Assert.Equal(
            "A-weighting IEC 61672 nominal frequency response",
            aWeighted.Parameters.WeightingMethod
        );
    ...
    public void CWeightingKeepsOneKilohertzCloseToZWeighting()
    {
        ...
        Assert.Equal(
            "C-weighting IEC 61672 nominal frequency response",
            cWeighted.Parameters.WeightingMethod
        );
    ...
    public void WeightingIsSuppressedForNonSoundPressureSignals()
    {
        ...
        Assert.Contains(
            aWeighted.Parameters.Limitations,
            limitation => limitation.Contains("BS/ISO 7196")
        );
    }
    ...
    )
    {
        ...
        for (var sampleIndex = 0; sampleIndex < sampleCount; sampleIndex++)
        {
            samples[sampleIndex] = (float)(
                0.5 * Math.Sin(2.0 * Math.PI * frequencyHz * sampleIndex / sampleRate)
            );
        }
    ...
    private static CpbBand FindBand(CpbAnalysis analysis, double centerFrequencyHz)
    {
        return analysis
            .Channels[0]
            .Bands.Single(band => Math.Abs(band.CenterFrequencyHz - centerFrequencyHz) < 0.01);
    }
}
