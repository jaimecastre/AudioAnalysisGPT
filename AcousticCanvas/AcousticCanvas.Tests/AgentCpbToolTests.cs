...
public sealed class AgentCpbToolTests
{
    ...
    public void FinalAnswerPromptCoversCpbLimitations()
    {
        ...
        Assert.Contains("Tinny = excess presence/high band energy", prompt, StringComparison.Ordinal);
        ...
        Assert.Contains("CPB evidence is band-level frequency-balance evidence", prompt, StringComparison.Ordinal);
        Assert.Contains("Do NOT claim IEC 61260 compliance", prompt, StringComparison.Ordinal);
        Assert.Contains("Do NOT use CPB as proof of an exact tonal peak frequency", prompt, StringComparison.Ordinal);
    ...
    public void EvidencePackageBuilderExtractsCpbEvidenceAndLimitation()
    {
        ...
        Assert.Equal("fft_bin_power_sum (nominal approximation, not IEC 61260)", evidence.Data["method"]);
        Assert.Equal("cpb_abcdef12", evidence.Data["dataRef"]);
        Assert.True(evidence.Data.ContainsKey("highestBands"));
        Assert.Contains(evidencePackage.Limitations, limitation => limitation.Contains("not IEC 61260", StringComparison.OrdinalIgnoreCase));
    }
                ...
                results = new[]
                {
                    new
                    {
                        ...
                        summary = new
                        {
                            highestBands = new[]
                            {
                                new { centerFrequencyHz = 1000.0, levelDb = -12.5, label = "1k" },
                            },
                        },
                    },
                    new
                    {
                        ...
                        summary = new
                        {
                            highestBands = new[]
                            {
                                new { centerFrequencyHz = 500.0, levelDb = -18.25, label = "500" },
                            },
                        },
                    },
                },
