#!/usr/bin/env python3
"""
Sound Quality Analysis Worker - Process Pool Version

This script runs in a loop, accepting multiple analysis requests via stdin
and writing results to stdout. Designed for process pooling to eliminate
startup overhead (~5-7 seconds per request).

Protocol:
- Input: JSON request + newline per request
- Output: JSON response + newline per response
- Exit cleanly on EOF or shutdown signal
"""
...
def fail(message):
    """Return error JSON to stdout and continue."""
...
def configure_runtime_environment():
    """Configure environment for headless matplotlib and fontconfig."""
...
def register_package_stub(package_name, package_directory):
    """Register a package stub for MoSQITo import."""
...
def import_metric_functions():
    """Import and return MoSQITo metric functions."""
...
def process_request(request, loudness_zwst, sharpness_din_from_loudness, roughness_dw):
    """Process a single analysis request and return result dict."""
    import numpy as np
    ...
    if region.shape[0] == 0:
        return {"error": "Selected sound-quality region contains no samples."}
...
def main():
    """Main loop - process requests until EOF or shutdown signal."""
    ...
    try:
        ...
        error_msg = f"MoSQITo sidecar dependency unavailable: {exception}"
    ...
    for line in sys.stdin:
        ...
        try:
            result = process_request(request, loudness_zwst, sharpness_din_from_loudness, roughness_dw)
            print(json.dumps(result), flush=True)
