...
- **Report filename includes region (✅ Done 2026-06-21):** `buildReportMarkdownDownload` in `reportDownload.ts` accepts optional `regionStartSeconds`/`regionEndSeconds` and appends a `_0.500s-2.000s` suffix to the downloaded `.md` filename when a region is active. `ExportReportModal` passes the active selection through.
- **Agent-mode region-scoped analysis (✅ Done 2026-06-21):** `AgentAskCommand` and `AgentAskRequest` extended with `ActiveSelectionStartSeconds`/`ActiveSelectionEndSeconds`. `AgentOrchestrator` forwards these to `AgentPlanner.PlanRequiredToolsAsync`. `BuildPlannerUserMessage` injects an "Active waveform selection: Xs – Ys" line into the LLM context, instructing the planner to include `startSeconds`/`endSeconds` in all tool arguments including `generate_report`. Frontend: `AgentAskRequest` type updated, `submitQuestion` in `useAgentAsk` accepts optional `activeSelection`, and `useChatInput` passes the Redux `activeSelection` through automatically on every send.
- **`ExtractIntArgument` fix (✅ Done 2026-06-21):** `ToolArgumentParser.ExtractIntArgument` previously only handled `JsonElement` inputs. Added plain .NET type fallbacks (`int`, `long` with range guard, `double` whole-number cast) — same pattern already applied to `ExtractDoubleArgument`.
- **`KeyFindingsSummary` audit (✅ Done 2026-06-21):** Removed misleading `onAnalyzeLoudestRegion` prop and "Analyze Loudest" button (no real implementation existed). Renamed `onInspectSpectrum` → `onRunSpectrum`. Gated `hasHighCrestFactor` suggestion on `findingsHaveBeenRun`. Fixed hardcoded `blue-0`/`gray-0`/`gray-1` backgrounds to use `var(--bg-elevated)`/`var(--bg-base)` design tokens for dark-mode compatibility.
- **`useWaveSurfer` / `WaveSurferDisplay` audit (✅ Done 2026-06-21):** Fixed `getTimeForClientX` to read actual rendered canvas width from WaveSurfer's shadow DOM `scrollContainer.scrollWidth` instead of estimating via `getWidth()` — fixes right-click timestamp drift when zoomed in. Clarified `clearSelection`/`setSelection` stubs with comments explaining they are placeholder no-ops overwritten by `WaveSurferDisplay`'s `useRegions` `useEffect`. Removed hardcoded `BACKGROUND_COLOR = '#ffffff'` from `WaveSurferDisplay` — waveform container now uses `var(--mantine-color-body)`, Y-axis canvas reads background from `getComputedStyle` at draw time.
